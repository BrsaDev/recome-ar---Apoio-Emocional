...
import { View, ForumTopic } from '../types';
...
export default function Admin({ navigate, forumTopics, onUpdateForumTopics }: Props) {
  ...
  const [totalVisitorCount, setTotalVisitorCount] = useState<number>(498);
  ...
  const loadLocalData = () => {
    ...
    if (savedUsers) {
      ...
      const defaultUsers: SimulatedUser[] = [
        { id: 'USR-8902', name: 'Carol S. Azevedo', email: 'carol.silva@gmail.com', mood: '😔 Triste', plan: 'premium', status: 'active', joinedAt: '2026-05-18' },
        { id: 'USR-3419', name: 'Lucas Menezes', email: 'lucas.mnez@outlook.com', mood: '😰 Ansioso', plan: 'vip', status: 'active', joinedAt: '2026-05-22' },
        { id: 'USR-7621', name: 'Renata Kunz', email: 'renata_k@hotmail.com', mood: '😶 Confuso', plan: 'basic', status: 'active', joinedAt: '2026-06-01' },
        { id: 'USR-1250', name: 'Amanda Vieira', email: 'amanda.v@gmail.com', mood: '😞 Sozinho', plan: 'free', status: 'active', joinedAt: '2026-06-03' },
        { id: 'USR-5582', name: 'Mateus Ferraz', email: 'mateus.ferr12@gmail.com', mood: '😰 Ansioso', plan: 'free', status: 'suspended', joinedAt: '2026-06-04' }
      ];
  ...
  const handleToggleUserStatus = (userId: string) => {
    const updated = simulatedUsers.map(user => {
      if (user.id === userId) {
        ...
        setApiLogs(prev => [...prev, `[Moderação] Usuário ${user.name} (${userId}) teve status alterado para: ${nextStatus.toUpperCase()}`]);
  ...
  const handleChangeUserPlan = (userId: string, newPlan: 'free' | 'basic' | 'vip' | 'premium') => {
    const updated = simulatedUsers.map(user => {
      if (user.id === userId) {
        setApiLogs(prev => [...prev, `[Faturamento] Plano do usuário ${user.name} alterado de ${user.plan.toUpperCase()} para ${newPlan.toUpperCase()}`]);
  ...
  const countBasic = simulatedUsers.filter(u => u.plan === 'basic').length;
  const estimatedMRR = (countPremium * 14.99) + (countVip * 5.99) + (countBasic * 0.99);
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-white border-b border-brand-blue/5 px-2.5 py-2 flex items-center space-x-1 shrink-0 overflow-x-auto no-scrollbar">
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold font-display uppercase tracking-wider transition-all cursor-pointer outline-none whitespace-nowrap ${
                activeTab === 'metrics' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:text-gray-750'
              }`}
            >
              <TrendingUp size={13} />
              <span>Geral</span>
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold font-display uppercase tracking-wider transition-all cursor-pointer outline-none whitespace-nowrap ${
                activeTab === 'users' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:text-gray-750'
              }`}
            >
              <UserCheck size={13} />
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold font-display uppercase tracking-wider transition-all cursor-pointer outline-none whitespace-nowrap ${
                activeTab === 'tickets' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:text-gray-750'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold font-display uppercase tracking-wider transition-all cursor-pointer outline-none whitespace-nowrap ${
                activeTab === 'rooms' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:text-gray-750'
              }`}
            >
              <Users size={13} />
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold font-display uppercase tracking-wider transition-all cursor-pointer outline-none whitespace-nowrap ${
                activeTab === 'forum' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:text-gray-750'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold font-display uppercase tracking-wider transition-all cursor-pointer outline-none whitespace-nowrap ${
                activeTab === 'supabase' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:text-gray-750'
              }`}
            >
              <Database size={13} />
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  ...
                  <div className="bg-white border border-brand-blue/5 rounded-2xl p-4 space-y-1 shadow-xs">
                    ...
                    <p className="text-2xl font-display font-black text-indigo-600">R$ {estimatedMRR.toFixed(2)}</p>
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          <div className="flex items-center space-x-1.5">
                            <span className="text-xs font-display font-black text-gray-900">{user.name}</span>
                          ...
                          <div className="flex items-center space-x-2 pt-0.5">
                            ...
                            <span className="text-[9px] text-gray-400 font-light">Cadastrado em {user.joinedAt}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="pt-2.5 border-t border-brand-blue/5 flex items-center justify-between flex-wrap gap-2 text-[10px]">
                        <div className="flex items-center space-x-1">
                          ...
                          {(['free', 'basic', 'vip', 'premium'] as const).map(tier => (
                            <button
                              key={tier}
                              onClick={() => handleChangeUserPlan(user.id, tier)}
                              className={`px-2 py-1 rounded text-[8.5px] uppercase font-mono font-bold transition-all cursor-pointer outline-none ${
                                user.plan === tier 
                        ...
                        <div className="flex items-center space-x-1">
                          <button
                            ...
                            className={`p-1.5 rounded-lg active:scale-90 transition-all cursor-pointer outline-none ${
                              user.status === 'active' 
                    ...
                    {tickets.map((ticket) => (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex items-center space-x-1.5">
                            <span className={`text-[8.5px] px-1.5 py-0.5 rounded-sm font-bold uppercase ${
                    ...
                    <div className="flex justify-between items-center py-1.5 border-t border-brand-blue/5">
                      ...
                      <div className="flex space-x-1.5">
                        <button
                          ...
                          className={`px-2 py-0.5 rounded text-[8.5px] font-mono font-bold ${
                            selectedSqlDialect === 'postgres' ? 'bg-indigo-600 text-white' : 'bg-brand-gray text-gray-400'
                          }`}
                        >
                          Raw SQL
                        </button>
                        <button
                          ...
                          className={`px-2 py-0.5 rounded text-[8.5px] font-mono font-bold ${
                            selectedSqlDialect === 'api-js' ? 'bg-indigo-600 text-white' : 'bg-brand-gray text-gray-400'
                          }`}
                        >
                          API Client
                        </button>
...
export async function insertSupportTicket(ticket) {
  ...
  return { data, error };
                    ...
                    <button
                      ...
                      className={`w-full py-3 rounded-xl text-xs font-bold transition-all shadow-sm active:scale-98 flex items-center justify-center space-x-1.5 cursor-pointer outline-none ${
